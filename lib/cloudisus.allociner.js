...
this.ingest = function(ahost, req, res, aURI) {
	...
	var jobData = querystring.stringify({ 'update' : 
		'	PREFIX void: <http://rdfs.org/ns/void#>\
			INSERT DATA {\
				<http://cloudisus.com/job/' + jobID + '> dc:title "Dataset submitted by ' + req.headers["user-agent"] + '" ;\
																void:dataDump <' + aURI +'> .\
			}'
	});
...
function registerContributor(req, res, datareadyproc){
	...
	var contributorData = querystring.stringify({ 'update' : 
		'	PREFIX cis: <http://purl.org/NET/cloudisus#>\
			INSERT DATA {\
				<http://cloudisus.com/contributor/' + contributorID + '> dc:title "Contributor from User Agent' + req.headers["user-agent"] + '" ;\
