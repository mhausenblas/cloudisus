...
this.ingest = function(req, res, aURI) {
	var jobID = hash.sha1(req.headers["user-agent"] + new Date());
	var datasetID = hash.sha1("_" + new Date());	
	var jobData = querystring.stringify({ 'update' : 
		'	PREFIX void: <http://rdfs.org/ns/void#>\
			PREFIX ciu: <http://purl.org/NET/cloudisus#>\
			INSERT DATA {\
				<http://cloudisus.com/job/' + jobID + '> rdf:type ciu:Job ;\
														 dc:title "Dataset submitted by ' + req.headers["user-agent"] + '" ;\
														 dc:created "' + dateFormat(new Date(), "isoUtcDateTime")  +'" ;\
														 ciu:input  <http://cloudisus.com/dataset/' + datasetID + '> .\
				<http://cloudisus.com/dataset/' + datasetID + '> rdf:type void:Dataset ;\
																 void:dataDump <' + aURI +'> .\
			}'
	});
	
	if(DEBUG) sys.debug("Got ingest URI " + aURI + " which is filed under job ID " + jobID);
...
// Lists all submitted jobs that are in the queue.
this.jobs = function(req, res) {
	var listJobs = '\
		PREFIX void: <http://rdfs.org/ns/void#>\
		PREFIX ciu: <http://purl.org/NET/cloudisus#>\
		SELECT * WHERE {\
			?job rdf:type ciu:Job ;\
				 dc:title ?label ;\
				 dc:created ?submitted ;\
				 ciu:input  ?ds .\
			?ds rdf:type void:Dataset ;\
				void:dataDump ?dump .\
		}';
	
	executeSelectQuery(listJobs, res, function(data, mediatype){ // the on-data-ready anonymous function
		if(data){ // we have the query result data in-mem
...
this.contribute = function(req, res, aURI) {
...
// Lists all registered contributors.
this.contributors = function(req, res) {
	var listContributors = '\
		PREFIX ciu: <http://purl.org/NET/cloudisus#>\
		SELECT * WHERE {\
			?contributor rdf:type ciu:Contributor ;\
						 dc:title ?label ;\
						 dc:created ?registered .\
		}';
	
	executeSelectQuery(listContributors, res, function(data, mediatype){ // the on-data-ready anonymous function
		if(data){ // we have the query result data in-mem
			writeData(res, data, mediatype); 
		}
		else { // error reading the data into memory, examine status
...
// Executes a  SELECT query against the central datastore
this.query = function(req, res, aURI) {
	var params = url.parse(aURI, true);
		
	if(DEBUG) sys.debug("Got query string: " + params.query.q);
	
	executeSelectQuery(params.query.q, res, function(data, mediatype){ // the on-data-ready anonymous function
		if(data){ // we have the query result data in-mem
			writeData(res, data, mediatype); 
		}
		else { // error reading the data into memory, examine status
...
function registerContributor(req, res, datareadyproc){
	...
	var contributorData = querystring.stringify({ 'update' : 
		'	PREFIX ciu: <http://purl.org/NET/cloudisus#>\
			INSERT DATA {\
				<http://cloudisus.com/contributor/' + contributorID + '> rdf:type ciu:Contributor ;\
...
function executeSelectQuery(qString, res, datareadyproc) {	
	...
	parseRemoteFile(dsQueryURI, 'application/json', DATASTORE_USERNAME, DATASTORE_PASSWORD, function(data, status, msg){ // the on-data-ready anonymous function
		if(data){ // we have the query result data in-mem
			datareadyproc(data, 'application/json'); 
		}
		else { // error reading the data into memory, examine status
			if(status === SERVER_404) {
				a404(res, "<div style='border: 1px solid red; background: #fafafa; font-family: monospace; font-size: 90%; padding: 3px;'>" + msg + "</div>");
